...
public class AuthData {
    ...
    public void setRefreshToken(final String token) {
        setToken(token, REFRESH_TOKEN_FILE);
    }
    ...
    public boolean isValid() {
        return (
            notBlank(this.props.getProperty(USERNAME_PROPERTY))
            && notBlank(this.props.getProperty(CLIENT_ID_PROPERTY))
            && notBlank(this.props.getProperty(CLIENT_SECRET_PROPERTY))
        );
    }

    private static boolean notBlank(final String v) {
        return v != null && !v.isEmpty() && v.chars().noneMatch(Character::isWhitespace);
    }
